			...
			if (not suc) then 
				if shared.VoidDev then
					task.spawn(function()
						...
						errorNotification("subbico | "..tostring(S_Name), debug.traceback(tostring(err)), 10)
					end)
				end
				task.spawn(function()
					repeat task.wait() until errorNotification ~= nil and type(errorNotification) == "function"
					if S_Name ~= "Not Specified" then
						if attemptedRestarts[S_Name] then 
							errorNotification('subbico | '..tostring(S_Name), "Restart failed!", 3)
							errorNotification("subbico | "..tostring(S_Name), "There was an error with this module. If you can please send the\n VW_Error_Log.json in your workspace to erchodev#0 or discord.gg/subbico", 10)
						else
							errorNotification('subbico | '..tostring(S_Name), "There was an error with this module. Attempting restart...", 3)
							attemptedRestarts[S_Name] = true
							local suc2, err2 = pcall(function() func(false) end)
							if suc2 then InfoNotification("subbico | "..tostring(S_Name), "Restart successfull!", 3); end
						end
					else
						errorNotification("subbico | "..tostring(S_Name), "There was an error with this module. If you can please send the\n VW_Error_Log.json in your workspace to erchodev#0 or discord.gg/subbico", 10)
					end
				...
				task.spawn(function()
					...
					if (not suc) then
						task.spawn(function()
							repeat task.wait() until errorNotification ~= nil and type(errorNotification) == "function" 
							pcall(function()
								errorNotification("subbico", "Failure loading "..tostring(v).." Error: "..tostring(err), 5)
							end)
						end)
					end
				end)
...
function mainapi:Save(newprofile)
	...
	if not suc then
		if errorNotification ~= nil and type(errorNotification) == "function" then
			pcall(function()
				errorNotification("subbico", "Failure saving your profile!", 3)
			end)
		end
	end
end
