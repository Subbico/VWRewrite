    ...
    ["Bedwars"] = {
        vapeGithubRequest = function(scripturl)
            if not isfile("vape/"..scripturl) then
                local suc, res = pcall(function() return game:HttpGet("https://raw.githubusercontent.com/Subbico/Subbico/main/"..scripturl, true) end)
                assert(suc, res)
...
VWFunctions.EditWL = function(argTable)
    ...
    if type(argTable) == "table" and argTable["api_key"] then
        ...
        if NewTag_text or NewTag_color or Roblox_Username then
            ...
            local url = "https://whitelist.Subbico.xyz/edit_wl"
            local a = request({
                Url = url,
                Method = 'POST',
                Headers = headers,
...
VWFunctions.fetchCheatEngineSupportFile = function(fileName)
    local url = "https://raw.githubusercontent.com/Subbico/VWCE/main/CheatEngine/"..tostring(fileName)
    local suc, res = pcall(function()
